...
var Account, AccountConfigError, BadRequest, ImapReporter, MSGBYPAGE, Mailbox, Message, NotFound, async, contentToBuffer, log, messageUtils, multiparty, querystring, stream_to_buffer, _, _ref,
...
module.exports.batchTrash = function(req, res, next) {
  ...
  if (!((ids != null ? ids.length : void 0) && ids.length > 0)) {
    return next(new BadRequest("no ids in request's body"));
  } else if (!accountID) {
    return next(new BadRequest("no accountID in request's body"));
  } else {
    return Account.findSafe(accountID, function(err, account) {
      var reporter;
      if (err) {
        return next(err);
      } else if (!account.trashMailbox) {
        ...
        return async.eachSeries(ids, function(id, cb) {
          return Message.moveToTrash(account, id, function(err) {
            if (err) {
          ...
          reporter.onDone();
          return log.info("BATCH TRASH " + reporter.id + " COMPLETE");
        });
      }
    });
  }
