...
var Account, ImapPool, Mailbox, Message, Scheduler, accountsByID, allAccounts, async, countsByMailboxID, imapPools, log, mailboxRefreshQueue, mailboxesByAccountID, mailboxesByID, onMessageCreated, onMessageDestroyed, orphanMailboxes, queueEndMarker, retrieveAccounts, retrieveCounts, retrieveMailboxes, retrieveTotalCounts, unreadByAccountID, _,
...
retrieveAccounts = function(callback) {
  ...
  return Account.all(function(err, cozyAccounts) {
    ...
    return async.mapSeries(cozyAccounts, function(account, next) {
      ...
      for (_i = 0, _len = cozyAccounts.length; _i < _len; _i++) {
        account = cozyAccounts[_i];
...
retrieveCounts = function(callback) {
  ...
  return Message.rawRequest('byMailboxRequest', options, function(err, rows) {
    var DATEFLAG, boxID, flag, row, _i, _len, _ref;
    if (err) {
      return callback(err);
    }
    for (_i = 0, _len = rows.length; _i < _len; _i++) {
...
retrieveTotalCounts = function(callback) {
  ...
  return Message.rawRequest('totalUnreadByAccount', {
    ...
    var accountID, count, row, _i, _len;
    if (err) {
      return callback(err);
    }
    for (_i = 0, _len = rows.length; _i < _len; _i++) {
...
exports.clientList = function() {
  ...
  return (function() {
    ...
    for (id in accountsByID) {
      _results.push(exports.getAccountClientObject(id));
    }
    return _results;
  })();
};
...
exports.getAllMailboxes = function() {
  var account, id, mailbox, out, _i, _len, _ref;
  out = [];
  for (id in accountsByID) {
    account = accountsByID[id];
    _ref = exports.getMailboxesByAccount(id);
...
exports.getFavoriteMailboxes = function() {
  var account, id, mailbox, out, _i, _len, _ref, _ref1;
  out = [];
  for (id in accountsByID) {
    account = accountsByID[id];
    _ref = exports.getMailboxesByAccount(id);
    for (_i = 0, _len = _ref.length; _i < _len; _i++) {
      mailbox = _ref[_i];
      if ((_ref1 = mailbox.id, __indexOf.call(account.favorites, _ref1) >= 0) || []) {
        out.push(mailbox);
      }
    }
  }
...
exports.getFavoriteMailboxesByAccount = function(accountID) {
  var account, mailbox, out, _i, _len, _ref, _ref1;
  out = [];
  account = exports.getAccount(accountID);
  _ref = exports.getMailboxesByAccount(accountID);
  for (_i = 0, _len = _ref.length; _i < _len; _i++) {
    mailbox = _ref[_i];
    if ((_ref1 = mailbox.id, __indexOf.call(account.favorites, _ref1) >= 0) || []) {
      out.push(mailbox);
...
exports.getNextBoxToRefresh = function() {
  var box;
...
exports.addMailbox = function(mailbox) {
  ...
  if (countsByMailboxID[_name = mailbox.id] == null) {
    countsByMailboxID[_name] = {
      unread: 0,
      total: 0,
      recent: 0
    };
  ...
  if (mailboxesByAccountID[accountID]) {
    ...
    return mailboxRefreshQueue.push(mailbox);
  } else {
...
Message.on('create', onMessageCreated = function(created) {
  var boxID, isRead, isRecent, uid, _ref;
  isRead = __indexOf.call(created.flags, '\\Seen') >= 0;
  isRecent = __indexOf.call(created.flags, '\\Recent') >= 0;
  _ref = created.mailboxIDs;
  for (boxID in _ref) {
    uid = _ref[boxID];
  ...
  if (isRead) {
    return unreadByAccountID[created.accountID] += 1;
  }
});

Message.on('delete', onMessageDestroyed = function(id, old) {
  var boxID, uid, wasRead, wasRecent, _ref;
  wasRead = __indexOf.call(old.flags, '\\Seen') >= 0;
  wasRecent = __indexOf.call(old.flags, '\\Recent') >= 0;
  _ref = old.mailboxIDs;
  for (boxID in _ref) {
    uid = _ref[boxID];
    countsByMailboxID[boxID].total -= 1;
  ...
  if (wasRead) {
    return unreadByAccountID[old.accountID] -= 1;
  }
});
