...
var Account, AccountConfigError, CONSTANTS, Compiler, ImapPool, ImapReporter, Mailbox, Message, NotFound, RefreshError, SMTPConnection, _, async, cozydb, log, nodemailer, notifications, ref, refreshTimeout,
...
ref = require('../utils/errors'), RefreshError = ref.RefreshError, NotFound = ref.NotFound;
...
Account.prototype.forgetBox = function(boxid, callback) {
  var attribute, changes, i, len, ref1;
  changes = {};
  ref1 = Object.keys(Mailbox.RFC6154);
...
Account.prototype.totalUnread = function(callback) {
  return Message.rawRequest('totalUnreadByAccount', {
    ...
    return callback(null, (results != null ? (ref1 = results[0]) != null ? ref1.value : void 0 : void 0) || 0);
  });
};
  ...
  return this.imap_refreshBoxes(function(err, toFetch, toDestroy) {
    ...
    if (onlyFavorites) {
      toFetch = toFetch.filter(function(box) {
        var ref1;
        return ref1 = box.id, indexOf.call(account.favorites, ref1) >= 0;
      });
    }
    toFetch = toFetch.filter(function(box) {
    ...
    return async.eachSeries(toFetch, function(box, cb) {
      return box.imap_fetchMails(limitByBox, firstImport, function(err, shouldNotif) {
        var ref1;
        if (err && -1 === ((ref1 = err.message) != null ? ref1.indexOf("Mailbox doesn't exist") : void 0)) {
...
Account.prototype.imap_scanBoxesForSpecialUse = function(boxes, callback) {
  var box, boxAttributes, changes, i, id, inboxMailbox, j, len, len1, priorities, ref1, type, useRFC6154;
  useRFC6154 = false;
  inboxMailbox = null;
  ...
  for (j = 0, len1 = boxes.length; j < len1; j++) {
    ...
    if (changes.favorites.length < 4) {
      if ((ref1 = box.id, indexOf.call(changes.favorites, ref1) < 0) && box.isSelectable()) {
