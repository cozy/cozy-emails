...
var Account, AccountConfigError, CONSTANTS, Compiler, ImapPool, ImapReporter, Mailbox, Message, NotFound, RefreshError, SMTPConnection, async, cozydb, log, nodemailer, notifications, refreshTimeout, _, _ref,
...
_ref = require('../utils/errors'), RefreshError = _ref.RefreshError, NotFound = _ref.NotFound;
...
Account.prototype.forgetBox = function(boxid, callback) {
  var attribute, changes, _i, _len, _ref1;
  changes = {};
  _ref1 = Object.keys(Mailbox.RFC6154);
...
Account.prototype.totalUnread = function(callback) {
  return Message.rawRequest('totalUnreadByAccount', {
    ...
    return callback(null, (results != null ? (_ref1 = results[0]) != null ? _ref1.value : void 0 : void 0) || 0);
  });
};
...
Account.prototype.imap_fetchMails = function(limitByBox, onlyFavorites, firstImport, callback) {
  ...
  return this.imap_refreshBoxes(function(err, toFetch, toDestroy) {
    var numToFetch, reporter, shouldNotifAccount;
    if (err) {
    ...
    if (onlyFavorites) {
      toFetch = toFetch.filter(function(box) {
        var _ref1;
        return _ref1 = box.id, __indexOf.call(account.favorites, _ref1) >= 0;
      });
    }
    toFetch = toFetch.filter(function(box) {
    ...
    return async.eachSeries(toFetch, function(box, cb) {
      return box.imap_fetchMails(limitByBox, firstImport, function(err, shouldNotif) {
        var _ref1;
        if (err && -1 === ((_ref1 = err.message) != null ? _ref1.indexOf("Mailbox doesn't exist") : void 0)) {
...
Account.prototype.imap_scanBoxesForSpecialUse = function(boxes, callback) {
  var box, boxAttributes, changes, id, inboxMailbox, priorities, type, useRFC6154, _i, _j, _len, _len1, _ref1;
  useRFC6154 = false;
  inboxMailbox = null;
  ...
  for (_j = 0, _len1 = boxes.length; _j < _len1; _j++) {
    ...
    if (changes.favorites.length < 4) {
      if ((_ref1 = box.id, __indexOf.call(changes.favorites, _ref1) < 0) && box.isSelectable()) {
