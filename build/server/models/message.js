...
var BadRequest, CONCURRENT_DESTROY, CONSTANTS, ImapPool, LIMIT_DESTROY, LIMIT_UPDATE, MSGBYPAGE, MailAdress, Mailbox, Message, NotFound, async, cozydb, htmlToText, log, mailutils, uuid, _, _ref,
...
_ref = require('../utils/errors'), NotFound = _ref.NotFound, BadRequest = _ref.BadRequest;
...
Message.getResultsAndCount = function(mailboxID, params, callback) {
  ...
  if (params.descending) {
    _ref1 = [params.after, params.before], params.before = _ref1[0], params.after = _ref1[1];
  }
  return async.series([
    function(cb) {
...
Message.getCount = function(mailboxID, params, callback) {
  ...
  return Message.rawRequest('byMailboxRequest', {
    ...
    return callback(null, ((_ref1 = rows[0]) != null ? _ref1.value : void 0) || 0);
  });
};
...
Message.prototype.markTwin = function(box, uid, callback) {
  ...
  if (_ref1 = box.id, __indexOf.call(hasTwin, _ref1) < 0) {
    return callback(null);
  }
  hasTwin.push(box.id);
...
Message.byMessageID = function(accountID, messageID, callback) {
  ...
  return Message.rawRequest('dedupRequest', {
    ...
    message = (_ref1 = rows[0]) != null ? _ref1.doc : void 0;
    if (message) {
...
Message.prototype.applyPatchOperations = function(patch, callback) {
  var boxOps, boxid, flagsOps, index, newflags, newmailboxIDs, operation, uid, _i, _j, _len, _len1, _ref1;
  log.debug(".applyPatchOperations", patch);
  newmailboxIDs = {};
  _ref1 = this.mailboxIDs;
  for (boxid in _ref1) {
    uid = _ref1[boxid];
...
Message.prototype.imap_applyChanges = function(newflags, flagsOps, newmailboxIDs, boxOps, callback) {
  ...
  return Mailbox.getBoxesIndexedByID(this.accountID, (function(_this) {
    return function(err, boxIndex) {
      var box, boxID, boxid, firstboxid, firstuid, _i, _len, _ref1;
      if (err) {
        return callback(err);
      }
        ...
        return async.series([
          function(cb) {
            ...
            return imap.multicopy(firstuid, paths, function(err, uids) {
              var destID, i, _j, _ref2;
              if (err) {
                return callback(err);
              }
              for (i = _j = 0, _ref2 = uids.length - 1; _j <= _ref2; i = _j += 1) {
...
Message.findConversationID = function(mail, callback) {
  var isReplyOrForward, key, keys, references, _ref1;
  ...
  } else if (((_ref1 = mail.normSubject) != null ? _ref1.length : void 0) > 3 && isReplyOrForward) {
...
Message.prototype.toClientObject = function() {
  var attachments, raw, _ref1;
  raw = this.toObject();
  if ((_ref1 = raw.attachments) != null) {
    _ref1.forEach(function(file) {
...
Message.prototype.moveToTrash = function(account, callback) {
  ...
  if (!trashBoxID) {
    return callback(next(new AccountConfigError('trashMailbox')));
