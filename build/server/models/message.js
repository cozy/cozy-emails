...
var CONCURRENT_DESTROY, CONSTANTS, ImapPool, LIMIT_DESTROY, LIMIT_UPDATE, MSGBYPAGE, Mailbox, Message, NotFound, americano, async, htmlToText, log, mailutils, sanitizer, uuid, _;
...
Message.updateOrCreate = function(message, callback) {
  ...
  if (message.id) {
    return Message.find(message.id, function(err, existing) {
      ...
      if (!existing) {
        return cb(new NotFound("Message " + message.id));
      }
  ...
  return Message.rawRequest('dedupRequest', {
    ...
    return async.eachLimit(rows, CONCURRENT_DESTROY, function(row, cb) {
      ...
      if (err && retries > 0) {
        ...
        return async.delay(4000, function() {
          retries = retries - 1;
...
Message.prototype.applyPatchOperations = function(patch, callback) {
  var boxOps, boxid, flag, index, newflags, newmailboxIDs, operation, uid, _i, _j, _len, _len1, _ref;
