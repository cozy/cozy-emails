...
var CONCURRENT_DESTROY, LIMIT_DESTROY, LIMIT_UPDATE, Mailbox, Message, Promise, americano, log, mailutils, uuid, _;
...
Message.getResultsAndCount = function(mailboxID, params) {
  var after, before, descending, options, pCount, pResults, sortField, _ref;
  before = params.before, after = params.after, descending = params.descending, sortField = params.sortField;
  if (descending) {
  ...
  pCount = Message.rawRequestPromised('byMailboxRequest', _.extend({}, options, {
    reduce: true,
    group_level: 2
  }));
  pResults = Message.rawRequestPromised('byMailboxRequest', _.extend({}, options, {
    reduce: false,
    limit: 3,
    include_docs: true
  })).map(function(row) {
