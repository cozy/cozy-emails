...
Message.getUnreadCountByAccount = function(accountID, callback) {
  return Message.rawRequest('accountUnread', {
    startkey: [accountID],
    endkey: [accountID, {}],
    reduce: true
  }, function(err, rows) {
    var counts, row, _i, _len;
    if (err) {
      return callback(err);
    }
    counts = {};
        ...
        return async.series([
          function(cb) {
            ...
            if (keywords.length === 0) {
              ...
              return imap.setKeywords(firstuid, newflags, cb);
            }
          }, function(cb) {
            var paths;
...
Message.prototype.moveToTrash = function(callback) {
  return Account.find(this.accountID, function(err, account) {
    var boxid, mailboxes, patch, trashBoxID;
    if (err) {
      return callback(err);
    }
    if (!account.trashMailbox) {
      return callback(new Error('no trash box'));
    }
