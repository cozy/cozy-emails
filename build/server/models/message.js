...
var CONCURRENT_DESTROY, CONSTANTS, ImapPool, LIMIT_DESTROY, LIMIT_UPDATE, MSGBYPAGE, MailAdress, Mailbox, Message, NotFound, _, async, cozydb, htmlToText, log, mailutils, uuid,
...
Message.prototype.imap_applyChanges = function(newflags, flagsOps, newmailboxIDs, boxOps, callback) {
  ...
  return Mailbox.getBoxesIndexedByID(this.accountID, (function(_this) {
    return function(err, boxIndex) {
      var box, boxID, boxid, firstboxid, firstuid, j, len, ref;
      if (err) {
        return callback(err);
      }
...
Message.prototype.moveToTrash = function(account, callback) {
  ...
  if (indexOf.call(mailboxes, trashBoxID) >= 0) {
    return callback(null);
  } else {
    patch = (function() {
      var j, len, results1;
